...
const ProductDetails: React.FC<props> = ({vendorName, unitRate, quantity, packaging, frieght, gst, selected}) =>{
    return (
        ...
        <div className="p-2 py-3 text-sm border border-[#090D11] text-center">{vendorName}</div>
        <div className="p-2 py-3 border text-sm text-bold border-[#090D11] text-center">{unitRate}</div>
        ...
        <div className="p-2 py-3 border text-sm text-bold border-[#090D11] text-center">{unitRate * quantity * (1 + (packaging + frieght + gst) / 100)}</div>
        </div>
        </>
    );
}
